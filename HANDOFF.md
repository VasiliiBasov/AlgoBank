...
- **🆕 SQL-трек создан (25.09):** `docs/sqldrills/SQL_TRACK.md` (мета+карта sql-01/02/03), профиль `application-sqldrill.yml` (своя H2 + Flyway-сид `V900__sqldrill_seed.sql`), 12 заготовок `src/main/resources/sqldrills/step01/q01..q12.sql` — **пишет ученик**, JUnit-автопроверка `SqlStep01Test` (инфра ментора: SqlDrillDb/SqlDrillRunner; гейт `-Dsqldrills=on`, чтобы CI оставался зелёным до решения). Приёмка: 12/12 зелёные + зачёт 80+
