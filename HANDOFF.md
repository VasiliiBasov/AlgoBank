...
- **Шаг:** 6 / 26 (DTO + валидация — **6A ✅ 92/100**: `CreateGreetingRequest` + `@Valid` POST `/api/greet` через BankGreeter-бин (static убран), тесты зелёные с поведенческими именами; далее 6B — `@RestControllerAdvice` + ProblemDetail; шаг 5 **✅ ЗАКРЫТ 20.09 ночью**: артефакты в `1b67465` + тег `step-05` + итог 5.0 ч в STATS)
...
- **Всего потрачено:** **~24.7 ч** (шаг 0–3 с ×1.85 коэффициентом: 14.5 ч; шаг 4 по факту без коэффициента: 1.4 ч; ночная сессия 17.09 по git-окну: 1.8 ч; вечер 18.09–ночь 19.09 (5C + задачи + тесты): 2.0 ч; день 19→20.09 по `Get-Date`: 5.0 ч нетто)
