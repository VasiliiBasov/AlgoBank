...
- **🎤 Mock-собеседование EGAR (25.09, день):** итог **46/100 → no hire (диагностический)** — волны 42/55/41. Главные провалы: SQL (20/100, 4 года без практики), checked/unchecked перепутаны, race condition ≠ deadlock, HashMap без коллизий/ресайза, нет DispatcherServlet/Jackson в рассказе о REST, «зарплата — чем выше, тем лучше» (красный флаг). **План на 2 вечера до боевого отклика:** SQL-блок шага 7 (q01–q12) + переписать рассказ о себе + узнать зарплатную вилку + проговорить эталоны вслух → повторный прогон 5 провальных вопросов (цель 80+)
- **🔄 Ротация дополнена (25.09):** SQL JOIN/GROUP BY · коллизии/ресайз HashMap · контракт equals/hashCode · checked vs unchecked · race vs deadlock · DispatcherServlet+Jackson цепочка · триединство @SpringBootApplication · идемпотентность + HTTP-коды · merge vs rebase · Docker слои/кэш (косвенно подтверждено, но формулировка просела — повторить)
- **🗄 SQL-поток интегрирован в курс (25.09):** постоянная практика внутри шагов 7–26, ритуалы как у алгозадач (красные тесты → ревью/разбор → баллы → зачёт; после зачёта ментор снимает гейт — тесты блока навсегда вливаются в прогон/CI). Карта: ш.7 SELECT×12 · ш.8 JOIN×8 · ш.9 GROUP BY×6 · ш.10 подзапросы×4 · ш.11 keyset/DML×5 · ш.12–17 закрепление 2–3/шаг · ш.18–19 продвинутый SQL · ш.20–26 разминки. Инфра (ментор): `SqlDrillDb`/`SqlDrillRunner` + профиль `application-sqldrill.yml` + сид `V900`; правила и карта: `docs/sqldrills/SQL_TRACK.md`
- **Далее по шагу 7:** SQL-блок 7 — q01–q12 (`mvn -Dtest=SqlStep07Test "-Dsqldrills=on" test`) → задачи (7easy «дубликаты транзакций через HashSet», 7medium «дедуп по сумме+дате») → зачёт (algo + SQL-ловушки) → тег `step-07`
