...
- **Шаг:** 6 / 26 (DTO + валидация — **6A ✅ 92/100**: `CreateGreetingRequest` + `@Valid` POST `/api/greet` через BankGreeter-бин (static убран), тесты зелёные с поведенческими именами; далее 6B — `@RestControllerAdvice` + ProblemDetail; шаг 5: артефакты ✅ в `1b67465`, ⏳ тег `step-05` + STATS-итог при закрытии дня)
...
- **Текущая активность (19→20.09, ночь):** шаг 6 — **6A закрыт (92/100)** за 4 итерации разбора (static-оксюморон, `@Component` на record, вызов в никуда, NPE в тесте). Далее: **6B** — `@RestControllerAdvice` + ProblemDetail (RFC 9457); задачи шага: палиндром IBAN (easy) + Luhn (medium). Вопрос-мостик к 6B: какие поля в теле дефолтного 400 от Spring. Хвосты ученику: мёртвые импорты (Ctrl+Alt+O), пробел в `@PostMapping (`. ⏳ Незакрытое: git-тег `step-05` + время дня в STATS
