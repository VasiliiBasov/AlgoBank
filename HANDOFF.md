...
- **Сделано 24.09:** nginx-цикл ✅; entities `Account`/`Transaction` + `AccountRepository` написаны учеником, **не закоммичены** (pom: +Lombok); документы синхронизированы: README (шаг 7), правила №17-18 в LEARNING_LOG, логи сессий 19.09→24.09 дословно перенесены в «🗄 Архив операционных логов», программа шагов 8/11/13 в хендбуке (+compose, +Flyway-эволюция, +MapStruct, +JSON-логи/prometheus-хвост)
- **✅ 7A закрыта 90/100 (24.09, вечер):** приёмка подтверждена — `Successfully applied 1 migration to schema "PUBLIC", now at version v1` + приложение поднялось и отвечает по HTTP. Ждёт: коммит (entities + pom + V1 + документы) + push
- **🔄 Ротация разминок:** (1) «Flyway vs Hibernate: кто валидирует схему и роняет старт» — №7/Q2 75/100 → ретест; (2) «Docker: что кэшируется локально = слои образа; что умирает = контейнер/процесс» — №7/Q3 75/100 → ретест. Luhn O(1) — снята с ротации (№7/Q1, 88/100). Разминка №7 закрыта (24.09, вечер)
- **Далее по шагу 7:** коммит+push → задачи (7easy «дубликаты транзакций через HashSet», 7medium «дедуп по сумме+дате») → зачёт → тег `step-07`
- **Хвосты:** закоммитить entities + pom; пометка «дубликаты в pom» закрыта — дублей нет (проверено 24.09)
- **⏸→▶ Сессия:** пауза 19:50:34 → 19:54:51 (4 мин) закрыта — на столе: **(1) коммит+push** (entities+pom+V1+документы — хвост!), **(2) алго-задачи шага 7**: easy `TransactionDuplicates.findDuplicates` и medium `TransactionDedup.dedupe` (ТЗ выданы, пакет `algo/step07`)
