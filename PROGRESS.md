...
**Текущий шаг:** 7 / 26 (JPA entities + Flyway: 7A миграция V1 ✅ 90/100 + commit `2d38ea2`, 7easy TransactionDuplicates ✅ 90/100; Docker Desktop ✅ 29.8.0, зачёт basics 80/100; далее 7medium `TransactionDedup` → зачёт шага → тег `step-07`). Шаг 6 **✅ закрыт 21.09** (6A 92 / 6B 90 / 6C 90 / задачи 93+92 / зачёты 80 → **88.9/100**, тег `step-06`)
**Процент:** 23% (6/26), шаг 6 **✅ закрыт** 21.09.2026 днём (88.9/100, тег `step-06`)
...
## 📚 Шаг 5 — план (Spring Boot REST basics)

**Разминка (правило №11):** ретест шага 4 — exhaustiveness в switch, when-guards, record pattern

**Программа:**
- 5A: `@SpringBootApplication` — автоконфигурация, запуск через IDEA, что реально происходит при старте
- 5B: первый `@RestController` + `@GetMapping`, embedded Tomcat, порт 8082
- 🪤 **Учебный момент:** Spring Security в classpath → первый запрос даст **401**. Сначала сами разбираемся почему, потом временный `permitAll()` с комментарием «настоящая настройка — шаг 9». Заранее НЕ чиним.
- 5C: `application.yml` — профили, порт, логирование
...
**Далее по 6:** задача easy ✅ палиндром IBAN (93/100; баг `hi-lo` — урок пограничных индексов) → задача medium ✅ **Luhn (92/100)**; потом 6C.
**6C ✅ 90/100 (зачёт 85/100), 21.09 день:** springdoc-openapi-starter-webmvc-ui v3.x (линейка для Boot 4) в pom; `@Tag("Greetings")` + `@Operation(summary)` на `GET /api/hello` и `POST /api/greet`; api-docs + Swagger UI + Try-it-out 400 `application/problem+json` как приёмка. Зачёт: роли «формат/библиотека/страница» и «спека-из-кода, всегда актуальна» ✅; решение «сваггер только на тест-профиле» ✅, но не названа причина (открытый сваггер в проде = разведка для атакующего; `springdoc.swagger-ui.enabled=false`/`api-docs.enabled=false`).
**🏁 Шаг 6 ЗАКРЫТ 21.09 днём — итог 88.9/100, тег `step-06`.** Следом шаг 7 (JPA + Docker basics #1) — блокер: Docker Desktop (WSL2) не установлен.
